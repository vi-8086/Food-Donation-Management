...
if (strlen($_SESSION['pgasoid']==0)) {
  ...
  {
    //Get Food Request ID, Status, Donor remark
    $frid=$_GET['frid'];
    $status=$_POST['status'];
    $donormsg=$_POST['donoremark']; 
  
   $query=mysqli_query($con, "update tblfoodrequests set status='$status',donorremark='$donormsg' where requestnumber='$frid'");
    if ($query) {
...
<!DOCTYPE html>
<head>
<title>Donor|Food Requests</title>
...
//Fetch Request ID and Donor ID from previous page
$fid=$_GET['frid'];   
  $donorid=$_SESSION['pgasoid'];

//Execute Query
$ret=mysqli_query($con,"select * from tblfoodrequests
join tblfood  on tblfood.foodID=tblfoodrequests.foodId
join tbldonor on tblfood.DonorID=tbldonor.ID
 where  tblfoodrequests.requestNumber='$fid' and tbldonor.ID='$donorid'");
...
<?php endif; ?>
  ...
  <table border="1" class="table table-bordered mg-b-0">
<?php if($row['status']=="Not Confirmed"){ ?>
<form method="post" name="submit">
  ...
  <tr>
    <th>Remark :</th>
    <td><textarea class="form-control" required name="donoremark"></textarea></td>
  </tr>
